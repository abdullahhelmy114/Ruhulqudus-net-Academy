...
export async function GET(request: Request, { params }: { params: { courseId: string } }) {
  ...
  try {
    const enrollment = await sql`SELECT 1 FROM enrollments WHERE user_uid = ${uid} AND course_id = ${params.courseId}`;
    if (enrollment.length === 0) return NextResponse.json({ error: 'Not enrolled' }, { status: 403 });

    const [course] = await sql`SELECT id, title, level FROM courses WHERE id = ${params.courseId}`;
    const lessons = await sql`
      ...
      WHERE l.course_id = ${params.courseId} AND l.status = 'approved'
      GROUP BY l.id
