        ...
        apps.map(app => (
          <div key={app.id} className="rounded-3xl border bg-card p-5 shadow-elegant flex justify-between items-center">
            ...
            <div className="flex gap-2">
              <button className="px-3 py-1 rounded-full border border-red-500/30 text-red-600">Reject</button>
              <button className="px-3 py-1 rounded-full bg-emerald-600 text-white">Approve</button>
            </div>
          </div>
        ))
      }
...
function CourseModerationTab() {
  ...
  return (
    <div>
      ...
      {/* استخدم نفس تصميم صفحة pending التي بنيناها سابقًا */}
      {courses.map(c => (
        <div key={c.id} className="rounded-3xl border bg-card p-5 mb-3 flex justify-between">
          ...
          <div className="flex gap-2">
            <button className="px-3 py-1 rounded-full border border-red-500/30 text-red-600">Reject</button>
            <button className="px-3 py-1 rounded-full bg-emerald-600 text-white">Publish</button>
          </div>
        </div>
      ))}
    </div>
    ...
    <div>
      ...
      <table className="w-full text-sm rounded-3xl border bg-card overflow-hidden">
        <thead className="bg-muted/40 text-xs uppercase">
          <tr>
            <th className="px-5 py-3 text-left">User</th><th>Role</th><th>Plan</th><th>Status</th><th>Joined</th>
          </tr>
        </thead>
        <tbody>
          {users.map((u: any) => (
...
function AIConfigurationTab() {
  return (
    <div>
      ...
      <p className="text-muted-foreground">Configure Nūr assistant system prompt, model, and knowledge base.</p>
      {/* يمكنك إعادة استخدام نفس واجهة AI من السابق */}
    </div>
  );
}